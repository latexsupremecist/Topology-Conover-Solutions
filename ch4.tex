...
\begin{solution}
\textbf{\underline{I probably did something wrong here.}} Obviously, $d(f,g) = d(g,f)$ and $d(f,g) \in \R^+$. Then, $|f(x) - g(x)| + |g(x) - h(x)| \geq |f(x) - h(x)|$ at every $x \in [0,1]$, so the integrals inherit the inequality, i.e. $d(f,g) + d(g,h) \geq d(f,h)$.
\end{solution}
