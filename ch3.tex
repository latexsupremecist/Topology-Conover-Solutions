...
\question{Let $(\Z,d)$ be the space of integers with the metric topology induced by the absolute value metric. Show that \textit{every} subset of $\Z$ is open in $(\Z,d)$. In particular, any set consisting of a single point is open in $(\Z,d)$.}
...
\question{Supplementary: No countable collection of open subsets of $[0,\Omega]$ intersect at $\{\Omega\}$ only.} \label{supp}
...
\question{Let $\{x_n:n\in\Z^+\}$ be the sequence in $\R$ defined by $x_n = \frac{1}{n}$. Does this sequence converge? To what? Prove it.}
...
\question{Let $\{x_n:n\in\Z^+\}$ be the sequence in $\R$ defined by $x_n = (-1)^n(\frac{1}{n})$. Does this sequence converge? To what? Prove it.}
...
\question{Let $\{x_n:n\in\Z^+\}$ be the sequence in $\R$ defined by $x_n = (-1)^n$. Does this sequence converge? To what? Prove it.} \label{alternatingsign}
...
\question{Is$(\R-\Q)\cap\{x\in\R:x=\frac{1}{\sqrt{n}} \text{ for } n \in \Z^+\}$ a closed subset of the \textit{irrationals} with the metric topology induced by the absolute value metric?}
...
\question{Let $[p_n = (x_n,y_n):n \in \Z^+\}$ be a sequence in $E^2$, the Euclidean plane. Prove that $p_n \rightarrow p = (x,y)$ if and only if both $x_n \rightarrow x$ and $y_n \rightarrow y$. (Thus a sequence in the plane converges if and only if it converges "coordinate-wise")}
...
\question{Prove that every real number can be written as the limit of a convergent sequence of \textit{rational} numbers, i.e., if $r \in \R$, exhibit a sequence $\{x_n:n\in\Z^+\} \subseteq \Q$ such that $x_n \rightarrow r$.}
...
\question{State precisely what it means when the point $x \in X$ is \textit{not} an accumulation point of the sequence $\{x_n:n\in\Z^+\} \subseteq X$.}
...
\question{Let $(X,d_1)$ and $(Y,d_2)$ be two metric spaces and let $f:D \subseteq X \rightarrow Y$. The $f$ is \textbf{continuous at} point $x_0 \in D$ if and only if whenever $\{x_n:n\in\Z^+\}$ is a sequence in $D$ that converges to $x_0$, then the sequence $\{f(x_n):n\in\Z^+\}$ converges to $f(x_0)$ in $Y$.}
...
\question{Let $S$ be a sequence in $\R$, and let $\Z^+$ have the discrete topology that it gets by saying that every point in $\Z^+$ is open. Prove that $S$ is continuous.}
...
\question{Determine all topologically different connected 1-manifolds that you can (\textit{connected} means all in one piece). Try to determine any essential differences between them.}
...
\question{Is a space that looks like a $Y$ a topological 1-manifold? Why or why not?}
...
\question{Give as many different examples of connected topological 2-manifolds as you can and indicate why you think they are different.}
...
\question{Find out what a Möbius strip and a Klein bottle are. Do you want to add anything to your answers to Problems 1 or 3 above?}
